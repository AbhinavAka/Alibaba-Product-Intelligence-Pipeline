...
def save_product_data(product_id):

    try:

        query = """
            INSERT INTO product_id_tracker (product_id)
            VALUES %s
            ON CONFLICT (product_id) DO NOTHING
        """

        data = [(pid,) for pid in product_id]
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as curr:
                ...
                with create_connection() as conn:
                    with conn.cursor() as curr:
                        ...
                        execute_values(curr, query, data, page_size=100)
                        conn.commit()

        print(f"{len(data)} products saved to DB")
